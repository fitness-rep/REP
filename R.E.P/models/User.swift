...
struct Goal: Codable, Identifiable {
    ...
    static func fromDictionary(_ data: [String: Any]) -> Goal? {
        ...
        // Handle targetDate that might come from Firestore as Timestamp
        let targetDate: Date
        if let timestamp = data["targetDate"] as? Timestamp {
            targetDate = timestamp.dateValue()
        } else if let date = data["targetDate"] as? Date {
            targetDate = date
        } else {
